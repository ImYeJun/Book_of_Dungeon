...
pygame.mixer.init() 
map_bgm00 = pygame.mixer.Sound("eight_bit.ogg")
#tro_bgm = pygame.mixer.music.load("bgm/main_lobby/AdhesiveWombat - Night Shade.mp3")
tro_bgm = pygame.mixer.Sound("AdhesiveWombat - Night Shade.ogg")
...
class Skill:
    def __init__(self,name = None,sort = "attack",type = "fire",value01 = 20,value02 = 30,img = None,txt = "Test"):
        self.name = name
        self.sort = sort
        self.type = type
...
normal_attack = Skill(name = "normal_attack",type= None , img="image/UI/skill/normal_attack.gif",txt="actually muscle wizard")   

blooming = Skill(name = "blooming",type= "dendro" , img="image/UI/skill/blooming.gif",txt="sprouting to the sky")
healing = Skill(name = "healing",type= "dendro" , img="image/UI/skill/healing.gif",txt="maybe it will not heal you but..")
leaf_blade = Skill(name = "leaf blade",type= "dendro" , img="image/UI/skill/leaf_blade.gif",txt="casuing grass irritation")

fire_arrow = Skill(name = "fire arrow",type= "fire" , img="image/UI/skill/fire_arrow.gif",txt="penetrating with flame")
hell_fire = Skill(name = "hell fire",type= "fire" , img="image/UI/skill/hell_fire.gif",txt="opening the hell gate")
explosion = Skill(name = "explosion",type= "fire" , img="image/UI/skill/explosion.gif",txt="i'm feeling like there's a egg")

tsunami = Skill(name = "tsunami",type= "water" , img="image/UI/skill/tsunami.gif",txt="overwhelming waves")
water_tornado = Skill(name = "water tornado",type= "water" , img="image/UI/skill/water_tornado.gif",txt="blowing away")
water_shot = Skill(name = "water shot",type= "water" , img="image/UI/skill/water_shot.png",txt="bubbles but big")
...
class StageSet:
    ...
    def stage_rebuild(self):
        ...
        txt012 = ScreenObject("txt","{} ~ {}".format(self.skill01.value01,self.skill01.value02),txt_color=Background,pos_x = self.skill01.img.pos_x,pos_y=450)
        ...
        txt022 = ScreenObject("txt","{} ~ {}".format(self.skill02.value01,self.skill01.value02),txt_color=Background,pos_x = self.skill02.img.pos_x,pos_y=910)
...
########################################################################################################################
                    ...
                    def calcuate(min,max,enemy_hp,enemy_type,a,b):
                        ...
                        if ch==0:
                            if a==enemy_type:
                                self.enemy_list[k].hp=enemy_hp-prota_damage*1.2
                            elif b==enemy_type:
                                self.enemy_list[k].hp=enemy_hp-prota_damage-10
                            else:
                                self.enemy_list[k].hp=enemy_hp-prota_damage
                            self.prota.hp=self.prota.hp-random.randint(5,10)
                            print(self.prota.hp)
                            print(self.enemy_list[k].hp)
                            print("o")
                        elif ch==1:
                            defense=random.randint(5,10)
                            if a==enemy_type:
                                self.enemy_list[k].hp=enemy_hp-prota_damage*1.2-defense
                            elif b==enemy_type:
                    ...
                    # battle_screen_background = ScreenObject("img",r"image\background\battle screen background.png",pos_x = 809,pos_y=500)
                    # screen.blit(battle_screen_background.img,battle_screen_background.rect)
                    screen.blit(self.enemy_list[k].img_set.img,self.enemy_list[k].img_set.rect)
                    screen_update()

                    while self.prota.hp >= 0 and self.enemy_list[k].hp >= 0:
...
##################################################################################################################
    ...
    def init(self):
        ...
        txt12 = ScreenObject("txt","Minimum:{}   Maximum:{}".format(self.prota.skill_01.value01,self.prota.skill_01.value02),pos_x = self.prota.skill_01.img.pos_x,pos_y=600,txt_color=Background)
        ...
        txt22 = ScreenObject("txt","Minimum:{}   Maximum:{}".format(self.prota.skill_02.value01,self.prota.skill_02.value02),pos_x = self.prota.skill_02.img.pos_x,pos_y=600,txt_color=Background)
